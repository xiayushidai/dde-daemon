...
func (a *Audio) applyConfig() {
	...
	for _, s := range a.ctx.GetSinkList() {
		if s.Name == cfg.Sink {
			...
			a.ctx.SetSinkVolumeByIndex(s.Index, cv) //通过pa设置音量。
			break
		}
	}
...
func (a *Audio) doSaveConfig() {
	...
	for _, sinkInfo := range ctx.GetSinkList() {
		logger.Warning("dosave Sink name ==:",sinkInfo.Name)
		logger.Warning("dosave Sink port ==:",sinkInfo.ActivePort.Name)
		logger.Warning("default name ===:",a.getDefaultSourceName())
		if a.getDefaultSinkName() != sinkInfo.Name {
			continue
		}
		logger.Warning("name equal")
		info.Sink = sinkInfo.Name
		info.SinkPort = sinkInfo.ActivePort.Name
		info.SinkVolume = sinkInfo.Volume.Avg()
		logger.Warning("save info sinkvolume",info.SinkVolume)
		break
	}

	for _, sourceInfo := range ctx.GetSourceList() {
		//logger.Warning("source list name :",sourceInfo.Name)
		//logger.Warning("source list port :",sourceInfo.ActivePort.Name)
		//logger.Warning("zjg name :",a.getDefaultSourceName())
	...
	logger.Warning("info sourceport :",info.SourcePort);
