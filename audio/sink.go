...
func (s *Sink) SetVolume(v float64, isPlay bool) *dbus.Error {
	...
	s.PropsMu.Lock()
	cv := s.cVolume.SetAvg(v) //设置音量
	s.PropsMu.Unlock()
	s.index=2
	s.audio.context().SetSinkVolumeByIndex(s.index, cv) //设置音量
	logger.Warning("this is isplay for index=",s.index)
	if isPlay {
		s.playFeedback()  //当我们调节音量大小时，可以立刻听到反馈的声音大小,注释掉以后就不会听到
	}
	return nil
}
...
func (s *Sink) update(sinkInfo *pulse.Sink) {
	...
	s.PropsMu.Unlock()
	logger.Warningf("old active port===%v,new active port===%v",oldActivePort,newActivePort)
	// TODO(jouyouyun): Sometimes the default sink not in the same card, so the activePortChanged inoccurate.
	// The right way is saved the last default sink active port, then judge whether equal.
	if s.audio.headphoneUnplugAutoPause && activePortChanged {   //耳机拔出时，暂停所有的声音。
...
func (s *Sink) playFeedback() {
	...
	name := s.Name
	s.PropsMu.RUnlock()
	logger.Warning("zjg device==",name)
