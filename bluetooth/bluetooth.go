...
type Bluetooth struct {
	...
	methods *struct {   //bluetooth暴露出去的各种方法。
...
func (b *Bluetooth) addDevice(dpath dbus.ObjectPath) {
	...
	logger.Warningf("330 dpath===%s",dpath)
	d := newDevice(b.systemSigLoop, dpath)
	b.adaptersLock.Lock()
	d.adapter = b.adapters[d.AdapterPath]  //AdapterPath一般为“/org/bluez/hci0”
	b.adaptersLock.Unlock()
	...
	b.config.addDeviceConfig(d.getAddress())  //一个是bluetooth 下config的device。

	b.devicesLock.Lock()
	b.devices[d.AdapterPath] = append(b.devices[d.AdapterPath], d)   //添加设备。  一个是bluetooth的device  key="/org/bluez/hci0"
	b.devicesLock.Unlock()

	connected := b.config.getDeviceConfigConnected(d.getAddress())  //这个地方的connected是不对的。
	logger.Warningf("connected === %d",connected)
	if connected || d.connected {
		logger.Warning("this is ---350")
		time.AfterFunc(25*time.Second, func() {  //为什么25秒之后才执行吗？
			d, _ := b.getDevice(dpath)
			logger.Warning("this is --- 352")
			if d == nil {
				return
			}
			logger.Warning("this is --- 356")
			adapterPowered, err := d.adapter.core.Powered().Get(0)
			if err != nil {
				logger.Warning(err)
				return
			}
			logger.Warning("this is --- 362")
			if !adapterPowered {
				return
			}
			logger.Warning("this is --- 366")
			...
			logger.Warningf("paired==%d,connected==%d",paired,connected)
			if paired && !connected {   //如果配对成功，且没有连接，那么进行自动连接操作。
...
func (b *Bluetooth) updateState() {
	...
	b.PropsMu.Lock()
	b.setPropState(uint32(newState))  //更改State的值，只要有一个设备是connected状态，那么这个state就是connected状态。
	b.PropsMu.Unlock()
}
...
func (b *Bluetooth) getPairedDeviceList() []*device {  //获取已经配对的list
	b.adaptersLock.Lock()
	defer b.adaptersLock.Unlock()
	b.devicesLock.Lock()
	defer b.devicesLock.Unlock()
