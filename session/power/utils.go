...
func (m *Manager) waitLockShowing(timeout time.Duration) {
	...
	for {
		...
		case _, ok := <-ticker.C:
			...
			logger.Debug("waitLockShowing tick",time.Now().Format("2006-01-02 15:04:05"))
...
func (m *Manager) doLock(autoStartAuth bool) {
	logger.Info("Lock Screen",time.Now().Format("2006-01-02 15:04:05"))
	...
	err = lockFrontObj.Call(lockFrontIfc+".ShowAuth", 0, autoStartAuth).Err   //调用lockFront的ShowAuth方法，显示锁屏界面。
	logger.Info("show info after",time.Now().Format("2006-01-02 15:04:05"))
	if err != nil {
