...
func newSleepInhibitor(login1Manager *login1.Manager) *sleepInhibitor {
	...
	_, err := login1Manager.ConnectPrepareForSleep(func(before bool) {
		logger.Info("login1 PrepareForSleep", before,time.Now().Format("2006-01-02 15:04:05"))
		...
		if before {
			...
			network.HandlePrepareForSleep(false)  //networkmanager
			bluetooth.HandlePrepareForSleep(false)  //Discoverable  不发现蓝牙设备.
