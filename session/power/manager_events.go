...
func (m *Manager) handleBeforeSuspend() {
	...
	logger.Debug("before sleep",time.Now().Format("2006-01-02 15:04:05"))
	...
	if locked {
		logger.Debug("before sleepi, already locked")
		return
	}
...
func (m *Manager) handleWakeup() {
	...
	logger.Debug("wakeup",time.Now().Format("2006-01-02 15:04:05"))
	if m.SleepLock.Get() {
		m.doLock(true)
	}
	...
	m.setDPMSModeOn()
	cmd := exec.Command("/usr/bin/xset dpms force on")
	err := cmd.Run()
	if err!=nil{
		logger.Warning(err)
	}
	logger.Warning("xset dpms force on")
