...
func (m *Manager) initConnectionManage() {
	...
	m.connections = make(map[string]connectionSlice)  //string 和conn的键值对
	m.connectionsLock.Unlock()

	_, err := nmSettings.ConnectNewConnection(func(cpath dbus.ObjectPath) {   //监听信号,新增连接
	...
	_, err = nmSettings.ConnectConnectionRemoved(func(cpath dbus.ObjectPath) {   //监听信号,删除连接
	...
	for _, cpath := range nmGetConnectionList() {  //获取当前所有连接配置列表,遍历列表
		m.addConnection(cpath)  //增加连接配置.
	}
}
...
func (m *Manager) addConnection(cpath dbus.ObjectPath) {
	...
	m.connectionsLock.Lock()
	defer m.connectionsLock.Unlock()
	conn, err := m.newConnection(cpath)  //如果新添加了一个connection,
	if err != nil {
		return
	}
	...
	default:
		m.connections[conn.connType] = append(m.connections[conn.connType], conn)  //将新的conn放到数组中.type做为key.
...
func (m *Manager) activateConnection(uuid string, devPath dbus.ObjectPath) (cpath dbus.ObjectPath, err error) {
	...
	cpath, err = nmGetConnectionByUuid(uuid)  //通过uuid获取接口
	if err != nil {
	...
	m.ActiveConnectDevpath = devPath  //设备服务path
	m.ActiveConnectUuid = uuid    //uuid
	m.ActiveConnectSettingPath = cpath  //连接服务的path.
	return
}
