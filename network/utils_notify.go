...
func notify(icon, summary, body string) {  //通知发生了变化.
	logger.Warning("notify icon: %q, summary: %q, body: %q", icon, summary, body)
	if !notifyEnabled {
	...
	nid, err := notification.Notify(0, "dde-control-center", nid,  //这边难道是通知dde-control吗?
