...
func (m *Manager) initDeviceManage() {
	m.devicesLock.Lock()
	m.devices = make(map[string][]*device)	//创建字符串和设备的键值对.
	m.devicesLock.Unlock()

	m.accessPointsLock.Lock()
	m.accessPoints = make(map[dbus.ObjectPath][]*accessPoint)	//创建字符串和无线热点的键值对.
	m.accessPointsLock.Unlock()
	//监听信号的机制是怎样的,到底怎样监听的.
	_, err := nmManager.ConnectDeviceAdded(func(path dbus.ObjectPath) {  //当添加设备时,dbus服务的属性会发生变化,触发回调函数.
		m.addDevice(path)	 //添加设备
	})
	if err != nil {
		logger.Warning(err)
	}
	_, err = nmManager.ConnectDeviceRemoved(func(path dbus.ObjectPath) {  //当设备删除时,dbus服务的属性发生变化,触发回调函数.
		notifyDeviceRemoved(path)	//发出设备被删除的通知.  这个通知给谁呢?
		m.removeDevice(path)	//删除设备.
	})
	if err != nil {
		logger.Warning(err)	//有错报错.
	}
	for _, path := range nmGetDevices() {  //通过执行org/freedesktop/NetworkManger的GetDevices()获取当前的设备.
		m.addDevice(path)	//添加设备.
	}
}
...
func (m *Manager) newDevice(devPath dbus.ObjectPath) (dev *device, err error) {
	...
	// connect signals
	_, err = dev.nmDev.ConnectStateChanged(func(newState uint32, oldState uint32, reason uint32) { //如果连接状态发生变化,调用回调函数
		logger.Debugf("device state changed, %d => %d, reason[%d] %s",
			oldState, newState, reason, deviceErrorTable[reason])   //100->110 ,reason:39. 一个用户获取请求要求断开连接.
