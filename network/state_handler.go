...
func (sh *stateHandler) watch(path dbus.ObjectPath) {
	...
	nmDev, err := nmNewDevice(path)  //创建新的device
	if err != nil {
		return
	}

	deviceType, _ := nmDev.DeviceType().Get(0)  //查看设备类型
	...
	_, err = nmDev.ConnectStateChanged(func(newState, oldState, reason uint32) {  //如果连接状态发生变化.执行func匿名函数.
		...
		switch newState {
		case nm.NM_DEVICE_STATE_PREPARE:   //设备已经准备好
			if data, err := nmGetDeviceActiveConnectionData(path); err == nil {
				dsi.aconnId = getSettingConnectionId(data)
				icon := generalGetNotifyDisconnectedIcon(dsi.devType, path)
				logger.Warning("--------[Prepare] Active connection info:", dsi.aconnId, dsi.connectionType, dsi.nmDev.Path_())
		...
		case nm.NM_DEVICE_STATE_ACTIVATED:   //设备进入actived状态
			icon := generalGetNotifyConnectedIcon(dsi.devType, path)
			msg := dsi.aconnId
			logger.Warning("zjg --------[Activated] Active connection info:", dsi.aconnId, dsi.connectionType, dsi.nmDev.Path_())
			if dsi.connectionType == connectionWirelessHotspot {
		...
		case nm.NM_DEVICE_STATE_FAILED, nm.NM_DEVICE_STATE_DISCONNECTED, nm.NM_DEVICE_STATE_NEED_AUTH,  //所有断开状态或者异常,进入下面的分支.
			nm.NM_DEVICE_STATE_UNMANAGED, nm.NM_DEVICE_STATE_UNAVAILABLE:
			logger.Warning("device disconnected, type %s, %d => %d, reason[%d] %s", getCustomDeviceType(dsi.devType), oldState, newState, reason, deviceErrorTable[reason])
			...
			// ignore if device's old state is not available,旧的状态不可获取,提前return.
			if !isDeviceStateAvailable(oldState) {
				logger.Debug("no notify, old state is not available")
				return
			}

			// notify only when network enabled,网络状态不可用,提前return.
			if !nmGetNetworkEnabled() {
			...
			logger.Warning("--------[Disconnect] Active connection info:", dsi.aconnId, dsi.connectionType, dsi.nmDev.Path_())
			var icon, msg string
			icon = generalGetNotifyDisconnectedIcon(dsi.devType, path)
			if len(msg) == 0 {
				switch reason {
