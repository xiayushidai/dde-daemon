...
func (v apSecType) String() string {  //三种加密方式.
	switch v {
	case apSecNone:
		return "none"
	case apSecWep:
...
//ssid是否存在. 服务标示集.可以将一个无线局域网分为几个需要不同身份验证的子网络.
...
func (m *Manager) activateAccessPoint(uuid string, apPath, devPath dbus.ObjectPath) (cpath dbus.ObjectPath, err error) {
	logger.Warning("ActivateAccessPoint: uuid=%s, apPath=%s, devPath=%s", uuid, apPath, devPath)

	cpath = "/"
	var nmAp *nmdbus.AccessPoint
	nmAp, err = nmNewAccessPoint(apPath)  //没有保存三个参数
	if err != nil {
		return
	}
	secType := getApSecType(nmAp)  //没有保存三个参数
	if uuid != "" {
		var needUserEdit bool
		needUserEdit, err = fixApSecTypeChange(uuid, secType)  //这个应该也没有保存
		if err != nil {
			return
		}
		if needUserEdit {
