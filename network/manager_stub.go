...
func (m *Manager) setPropVpnEnabled(value bool) {
	m.VpnEnabled = value
	err := m.service.EmitPropertyChanged(m, "VpnEnabled", value)  //发送属性改变的信号.
	if err != nil {
		logger.Warning(err)
	}
}
...
func (m *Manager) updatePropActiveConnections() {
	...
	m.service.EmitPropertyChanged(m, "ActiveConnections", m.ActiveConnections)  //发出属性改变的信号.
}
...
func (m *Manager) updatePropConnections() {
	...
	m.service.EmitPropertyChanged(m, "Connections", m.Connections)  //发出属性变更的信号. 谁接收呢?
}
