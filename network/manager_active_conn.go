...
func (m *Manager) initActiveConnectionManage() {
	...
	err := dbusutil.NewMatchRuleBuilder().  //建立匹配规则.有信号时,将信号带来的参数和规则进行匹配, 匹配成功,才会调用回调函数.
		Type("signal").     //type = "signal"
		Sender(senderNm).	//sender="org.freedesktop.NetworkManager"
		Interface(interfaceDBusProps). //interface="org.freedesktop.DBus.Properties"
		Member(memberPropsChanged).	//member="PropertiesChanged"  属性改变.  这是一个信号.
		ArgNamespace(0, interfaceActiveConnection).Build().	//arg0namespace="org.freedesktop.NetworkManager.Connection.Active"
		AddTo(m.sysSigLoop.Conn())  //将匹配规则添加到信号的conn中.
	if err != nil {
		logger.Warning(err)
	}
	...
	err = dbusutil.NewMatchRuleBuilder().
		Type("signal").
		Sender(senderNm).	//"org.freedesktop.NetworkManager"
		Interface(interfaceVpnConnection).  //"org.freedesktop.NetworkManager.VPN.Connection"
		Member(memberVpnStateChanged).Build().
		AddTo(m.sysSigLoop.Conn())
	if err != nil {
		logger.Warning(err) //打印错误日志.
	}

	sysSigLoop.AddHandler(&dbusutil.SignalRule{  //添加处理函数   监听org/freedesktop/NetworkManager/ActiveConnection  PropertiesChanged
		Name: interfaceDBusProps + "." + memberPropsChanged,
	}, func(sig *dbus.Signal) {
		if strings.HasPrefix(string(sig.Path),  //信号的path中是否包含下面字段.
			"/org/freedesktop/NetworkManager/ActiveConnection/") &&  //表示该信号是符合的.
			len(sig.Body) == 3 {

			ifc, ok := sig.Body[0].(string)   //取得body的第一个字符.
			if !ok {
				return
			}

			if ifc != interfaceActiveConnection {  //如果接口不对,提前return.
				return
			}

			props, ok := sig.Body[1].(map[string]dbus.Variant)  //判断第二个字符是否为string和Variant键值对.
			if !ok {
				return
			}
			...
			logger.Debugf("zjg %s active connection %s state changed %v",time.Now().Format("20060102150405"),sig.Path, state)  //对于有线网络,代码会走到这边.
			m.updateActiveConnState(sig.Path, state)   //如果active的state发生变化,更新变化.
		}
	})

	// handle notification for vpn connections
	sysSigLoop.AddHandler(&dbusutil.SignalRule{
		Name: interfaceVpnConnection + "." + memberVpnStateChanged,  //vpn的state发生改变
	}, func(sig *dbus.Signal) {
		if strings.HasPrefix(string(sig.Path),
			"/org/freedesktop/NetworkManager/ActiveConnection/") &&
			len(sig.Body) >= 2 {

			state, ok := sig.Body[0].(uint32)  //当前的状态
			if !ok {
				return
			}
			reason, ok := sig.Body[1].(uint32) //状态改变的原因.
			if !ok {
				return
			}
			logger.Debug(sig.Path, "vpn state changed", state, reason)
			m.doHandleVpnNotification(sig.Path, state, reason)  //通知发生变化.
		}
	})
}
